...
void Kalman_Unscented::compute_sigma() const
{
    int i, j;
...
void Kalman_Unscented::get_matrices() const
{
    int i, j, k;
    ...
    math::invert_matrix(P_aux, P_y_aux, m.forward_size);
...
void Kalman_Unscented::get_update(std::vector<double> &mes, std::vector<double> &upd_vec, std::vector<double> &upd_cov) const
{
    int i, j, k;
...
void Kalman_Unscented::update_covariance(std::vector<double> &upd_cov, double factor)
{
   ...
   math::matrix_square_root(S, P, m.num_pars);
}
}; // filter